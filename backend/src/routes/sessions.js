 ...
 *
 * Body: { carnet, grupo, semestre, curso, problemId, solved }
 *
 * Lógica de upsert:
...
router.post("/", async (req, res) => {
  const { carnet, grupo, semestre, curso, problemId, solved } = req.body;

  if (!carnet || !grupo || !semestre || !curso || !problemId) {
    return res.status(400).json({
      success: false,
      error: "Faltan campos requeridos: carnet, grupo, semestre, curso, problemId",
    });
  }
  ...
  try {
    const result = await pool.query(
      `INSERT INTO exercise_sessions (carnet, grupo, semestre, curso, problem_id, attempts, solved)
      ...
      [carnet, grupo, Number(semestre), curso, problemId, solved],
    );
